...
[![Build Status](https://travis-ci.com/MattClarkson/scikit-surgerypclcpp.svg?branch=master)](https://travis-ci.com/MattClarkson/scikit-surgerypclcpp)
...
would be:
``` cmake
git clone --recursive https://github.com/MattClarkson/scikit-surgerypclcpp
mkdir scikit-surgerypclcpp-Build
cd scikit-surgerypclcpp-Build
ccmake ../scikit-surgerypclcpp
make
```
...
```StartVS_Debug.bat``` or ```StartVS_Release.bat``` in the ```SKSURGERYPCLCPP-build``` folder.
...
* Clone scikit-surgerypclcpp, using ```--recursive```.
...
```
import sksurgerypclpython as mp
